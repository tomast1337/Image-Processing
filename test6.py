...
for i in range(0, 255):
    hist_especific[i] = np.cos(i/64) * 255
...
hist_especific_cumm = hist_especific.cumsum() * 255

#map the gray values to the new histogram
for i in range(0,image.shape[0]):
    for j in range(0,image.shape[1]):
        mapped_img[i,j] = 255 * hist_especific_cumm[gray[i,j]]
